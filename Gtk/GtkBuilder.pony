...
class GtkBuilder is GtkBuilderInterface
  ...
  new create() =>
    obj = Gtk4Builder.gtk_builder_new()
...
interface GtkBuilderInterface is GObjectInterface
  ...
  fun ref add_from_file(filename: String) =>
    Gtk4Builder.gtk_builder_add_from_file(getobj(), filename.cstring(), Pointer[NullablePointer[GError]])

  fun ref get_object(name: String): NullablePointer[GObject] =>
    Gtk4Builder.gtk_builder_get_object(getobj(), name.cstring())
